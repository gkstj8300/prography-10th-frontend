...
export const StepForm: React.FC<Props> = ({
    ...
    const applicationFields = [
        '프론트엔드', '백엔드', '디자인', 'iOS', '안드로이드', 'Product Owner'
    ] as const;

    const isNextEnabled = useMemo(() => {
        const { consentAgreed, name, email, phoneNumber, applicationField } = applicationData;
        ...
        <StepFormContainer>
            ...
            {step === 2 && (
                <StepSection>
                    ...
                    <TextInput title='성함' placeholder='성함을 입력해주세요' type='text' name='name' isRequired value={applicationData.name} onChange={handleInputChange} />
                    <TextInput title='이메일 주소' placeholder='예시: prography@gmail.com' type='email' name='email' isRequired value={applicationData.email} onChange={handleInputChange} />
                    <TextInput title='휴대폰 번호' placeholder='예시: 010-1234-5678' type='text' name='phoneNumber' isRequired value={applicationData.phoneNumber} onChange={handleInputChange} />
                </StepSection>
            )}
            {step === 3 && (
                <StepSection>
                    <Title>지원 정보</Title>
                    <SubTitle>지원하고자 하는 분야를 선택해주세요 <RequiredValue>*</RequiredValue></SubTitle>
                    <CheckboxWrapper>
            ...
            <ButtonWrapper>
                <Button theme='prev' onClick={() => handleStepClick('prev')} disabled={step === 1}>
                    이전
                </Button>
                {step !== 3 ? (
                    <Button theme='next' onClick={() => handleStepClick('next')} disabled={!isNextEnabled}>
                        다음
                    </Button>
                ) : (
